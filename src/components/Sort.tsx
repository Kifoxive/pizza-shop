...
const sortList: SortItem[] = [{ name: "популярности ↑", sortProperty: SortPropertyEnum.RATING_DESC },
      ...
      <div ref={sortRef} className="sort">
         ...
         </div>
         {
            open && <div className="sort__popup">
               <ul>{
                  sortList.map((obj, index) => <li onClick={() => onClickListItem(obj)} className={value.sortProperty === obj.sortProperty ? 'active' : ''} key={index}>{obj.name}</li>)
               }</ul>
            </div>
         }
      </div>
